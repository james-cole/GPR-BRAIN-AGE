...
class GaussianProcessRegression(object):
	...
	def __init__(self,num_data,num_test,dim_input,dim_output,lista_initializare,age_mean,num_validation,num_patch):
		...
		self.X_train = tf.placeholder(tf.float64,shape=(num_data,dim_input),name='X_train')
		self.Y_train = tf.placeholder(tf.float64,shape=(num_data,dim_output),name='Y_train')

		self.X_validation = tf.placeholder(tf.float64,shape=(num_validation,dim_input),name='X_validation')
		self.Y_validation = tf.placeholder(tf.float64,shape=(num_validation,dim_output),name='Y_validation')

		self.X_test = tf.placeholder(tf.float64,shape=(num_test,dim_input),name='X_test')
		self.Y_test = tf.placeholder(tf.float64,shape=(num_test,dim_output),name='Y_test')
		self.variance_output = tf.Variable(0.0,dtype=tf.float64,name='likelihood_variance',trainable=True)
		self.variable_summaries(self.variance_output,name='var_output')
		lista_initializare = tf.log(lista_initializare)            
		self.variance_kernel = tf.Variable(0.0,dtype=tf.float64,name='variance_kernel')
		self.variable_summaries(self.variance_kernel,name='var_kernel')
		self.lengthscales = tf.Variable(lista_initializare,dtype=tf.float64,name='self.lengthscales')
		self.variable_summaries(self.lengthscales,name='lengthscales')
		tf.add_to_collection('variance_kernel',self.variance_kernel)
		tf.add_to_collection('lengthscales',self.lengthscales)
		self.similarity_matrix = tf.Variable(tf.zeros(shape=(num_data,num_data),dtype=tf.float64),trainable=False,name='sim_matrix')
		tf.add_to_collection('sim_matrix',self.similarity_matrix)
		self.K = tf.placeholder(tf.float64,shape=(num_data,num_data),name='placeholder_sim_matrix')
		self.assign_op = tf.assign(self.similarity_matrix,self.K)

		self.saver = tf.train.Saver(var_list=tf.get_collection('variance_kernel')+tf.get_collection('lengthscales')+tf.get_collection('sim_matrix'))
	...
	def session_TF(self,X_training,Y_training,X_testing,Y_testing,X_validation,Y_validation):
		...
		rmse_training = tf.sqrt(tf.reduce_mean(tf.squared_difference(self.Y_train,predictions_training)))
		tf.summary.scalar('rmse_training',tf.squeeze(rmse_training))
		rmse_validation = tf.sqrt(tf.reduce_mean(tf.squared_difference(self.Y_validation,predictions_validation)))
		tf.summary.scalar('rmse_validation',tf.squeeze(rmse_validation))
		merged = tf.summary.merge_all()
		try:
			os.mkdir('./tensorboard/patch_'+str(self.num_patch))
		except:
			pass

		train_writer = tf.summary.FileWriter('./tensorboard/patch_'+str(self.num_patch),self.sess.graph)
		...
		for i in range(6000):
			...
			print('at iteration '+str(i) + ' we have nll : '+str(costul_actual) +' **** training rmse : '+str(rmse_training_now)+ ' **** rmse validation : '+str(rmse_validation_now))

		
		predictii  = self.sess.run(predictions,feed_dict={self.X_test:X_testing,self.X_train:X_training,self.Y_train:Y_training})
		...
		print(mae_now)
		text_printat=''
		text_printat+='****RMSE at testing time****'
		text_printat+='\n'
		text_printat+=str(rmse_now)
		text_printat+='\n'
		text_printat+='****MAE at testing time****'
		text_printat+='\n'
		text_printat+=str(mae_now)
		with open('./results/patch_'+str(self.num_patch),'w') as file:
		...
		self.saver.save(self.sess,'./logs/patch_'+str(self.num_patch)+'/GPR_model_'+str(self.num_patch))
...
if __name__ == '__main__':
	...
	parser.add_argument('--num_patch',type=int, help='an integer for the accumulator')

	args = parser.parse_args()
	num_patch = args.num_patch

	try:
		os.mkdir('./logs/patch_'+str(self.num_patch))
	except:
		pass
	...
	print('shape of Y')
	print(Y_total.shape)
	X_total = np.genfromtxt('/home/sebict/brain_data/generalized_patches_grey_matter_60_70_60/X_original_patch_'+str(num_patch)+'.txt',dtype=np.float64)
	print('shape of X')
	print(X_total.shape)
	...
	t4=time.time()
	obiect =  GaussianProcessRegression(num_data = X_training.shape[0],num_test=X_testing.shape[0],dim_input=X_training.shape[1],dim_output=Y_training.shape[1],lista_initializare=lista_initializare,age_mean=age_mean,num_validation=X_validation.shape[0],num_patch=num_patch)
